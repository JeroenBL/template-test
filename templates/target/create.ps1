...
try {
    ...
    if ($dryRun -eq $true) {
        Write-Warning "[DryRun] $action {connectorName} account for: [$($p.DisplayName)], will be executed during enforcement"
    }
    ...
    if (-not($dryRun -eq $true)) {
        switch ($action) {
            ...
            'Update-Correlate' {
                Write-Verbose 'Updating and correlating {connectorName} account'
                break
            }
