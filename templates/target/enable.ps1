...
try {
    ...
    if ($dryRun -eq $true) {
        Write-Warning "[DryRun] Enable {connectorName} account for: [$($p.DisplayName)] will be executed during enforcement"
    }

    # Process
    if (-not($dryRun -eq $true)) {
        Write-Verbose "Enabling {connectorName} account with accountReference: [$aRef]"
    ...
    if ($($ex.Exception.GetType().FullName -eq 'Microsoft.PowerShell.Commands.HttpResponseException') -or
        ...
        $auditMessage = "Could not enable {connectorName} account. Error: $($ex.Exception.Message)"
